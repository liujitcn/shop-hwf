...
import type { UserCart } from '@/rpc/app/user_cart'
...
const onDeleteCart = (id: number) => {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        // 后端删除单品
        await defUserCartService.DeleteUserCart({ value: id })
        // 重新获取列表
...
// 修改选中状态-单品修改
const onChangeSelected = (item: UserCart) => {
  // 前端数据更新-是否选中取反
  item.isChecked = !item.isChecked
  // 后端数据更新
  defUserCartService.SetUserCartStatus({ id: item.id, isChecked: item.isChecked })
}
...
const selectedCartListMoney = computed(() => {
  return selectedCartList.value
    .reduce((sum, item) => sum + (item.num * item.price) / 100, 0)
    .toFixed(2)
})
